...
| **Конфиг** | Не хранить креды в коде (doxoz, minhalawais) — у нас в .env только GOOGLE_SHEETS_ID и GOOGLE_CREDENTIALS_PATH; остальное (Telegram, VFS proxy, тайминги) — лист Settings в Sheets. |
...
## Рекомендуемый метод для us-visa-bot

По результатам разбора репозиториев ниже выбран один комбинированный подход и что из кого брать.

### Сводка методов в проектах

| Метод | Кто использует | Плюсы | Минусы для нас |
|-------|-----------------|-------|-----------------|
| **Только fetch + cheerio** | наш vfsglobal.ts | Без браузера, быстро | На VFS почти всегда упирается в Cloudflare; без sitekey в HTML не пройти |
| **Headless-браузер на весь сценарий** | ranjan (Playwright), doxoz (DrissionPage), minhalawais (Selenium) | Обход Cloudflare, один контур, стабильный парсинг после рендера | Тяжелее, нужен браузер; у ranjan нет солвера капчи |
| **Браузер только для CF, дальше fetch** | наш test-vfs-captcha + идея «cookies reuse» | Меньше времени в браузере | Сложный обмен cookies/session между браузером и fetch; хрупко |
| **Stealth-браузер** | ranjan (playwright-stealth), barrriwa (BAS) | Лучше проходимость Cloudflare | У нас уже есть puppeteer-extra-plugin-stealth |
| **Солвер капчи (2Captcha и др.)** | мы, mominurr (reCAPTCHA), IndiaTransform (заявлено) | Автоматический логин при капче на форме | — |
| **Фабрика по стране/маршруту** | ranjan (VfsBotDe, VfsBotIt) | Разные селекторы и шаги под каждую страну VFS | Нужно поддерживать несколько «стратегий» |
| **Парсинг слотов: div.alert** | ranjan (DE) | Простой, одна строка с датой | Не везде есть такой блок |
| **Парсинг слотов: календарь (fc-daygrid-*)** | doxoz, minhalawais | Универсально для страниц с календарём | Нужны стабильные селекторы под нашу локаль |
| **Парсинг: «Earliest Available Slot»** | Hit2theMo | Очень простой текст | Хрупко, ломается при смене вёрстки/языка |
| **Pre-login: cookie consent** | ranjan (Reject All) | Меньше лишних попапов | Обязательный шаг на многих локалях |

### Выбор: комбинированный подход

**Оптимально для нас:** в основе **полный сценарий в headless-браузере (Puppeteer)** с stealth и 2Captcha, по образцу **ranjan-mohanty** по шагам, но на нашем стеке (Node/TS) и с уже имеющимися солверами капчи.

Причины:

1. **Cloudflare на VFS** почти всегда режется только браузером (или редким случаем с sitekey в HTML). Делать логин и проверку слотов в одном браузере проще и надёжнее, чем таскать cookies в fetch.
2. **ranjan** даёт проверенную последовательность: pre_login (cookies) → login → «Start New Booking» → выбор центра/категории/подкатегории → чтение дат (alert или календарь). Эту логику переносим в наш VfsGlobalProviderAdapter на Puppeteer.
3. **Капчу** мы уже умеем решать (2Captcha: Turnstile, reCAPTCHA, image) — у ranjan этого нет, у нас есть, это наше преимущество.
4. **Страны/локали** у VFS отличаются полями и селекторами — как у ranjan делаем **стратегии по локали** (или по паре source–destination), а не один жёсткий сценарий.
5. **Обнаружение слотов:** сначала проверять блок типа **div.alert** (как в ranjan DE), при отсутствии — парсить **календарь** (классы fc-daygrid-*, как у doxoz/minhalawais), чтобы не зависеть от одного формата.

### Что взять из кого

| Источник | Что взять |
|----------|-----------|
| **ranjan-mohanty** | Последовательность: pre_login → login → Start New Booking → выбор параметров (centre, category, subcategory) → парсинг дат (div.alert). Фабрика/стратегии по стране. Конфиг (параметры записи) не в коде. |
| **mominurr** | Идея явного обхода Cloudflare + reCAPTCHA + (опционально) прокси; у нас уже 2Captcha и браузер, прокси при необходимости добавить позже. |
| **doxoz / minhalawais** | Парсинг календаря (fc-daygrid-event, fc-toolbar-title) как запасной вариант, когда нет div.alert. |
| **berkay (issues)** | Учитывать «Mandatory field cannot be left empty» после reCAPTCHA — дождаться применения токена/скрытого поля перед submit. |
| **vlaim** | Не для букинга; напоминание про сценарий «track application» (статус по номеру заявки) — отдельная команда/провайдер при необходимости. |

### Итоговая схема для нашего VFS

1. **Единый сценарий в Puppeteer** (с puppeteer-extra-plugin-stealth): открыть страницу логина → pre_login (cookie consent) → ввод email/password → при появлении капчи (Turnstile/reCAPTCHA/image) — 2Captcha → submit → переход к «Start New Booking».
2. **Параметры записи** (центр, категория, подкатегория и т.п.) брать из конфига/пользователя (как у ranjan), не хардкодить.
3. **Стратегии по локали** (например `rus/en/fra`, `ind/en/deu`): разные селекторы и при необходимости порядок шагов; внутри VfsGlobalProviderAdapter вызывать стратегию по `countryCode`/locale.
4. **Получение дат:** сначала искать блок с текстом даты (например div.alert), если нет — парсить календарь (fc-daygrid-event и т.п.).
5. **Порт VisaProvider** не менять: реализация (VfsGlobalProviderAdapter) внутри выполняет шаги 1–4 и возвращает даты/время через существующие методы `getAvailableDates` / `getAvailableTime`; при необходимости букинг — `book` в том же браузере (те же сессии/cookies).

Так мы сохраняем архитектуру (порты/адаптеры), используем сильные стороны других проектов (браузер, структура шагов, парсинг слотов) и уже имеющиеся 2Captcha и stealth.
