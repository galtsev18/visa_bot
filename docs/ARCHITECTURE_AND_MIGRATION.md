...
### Текущий прогресс (на момент миграции)

- **Фаза 0:** ESLint, Prettier, TypeScript в проекте (частично). Контракты в `ports/` — в заделе.
- **Фаза 3 (выполнена):** Use cases вынесены в `src/application/`:
  - `startMonitor.js` — инициализация кэша, уведомление "Monitor started";
  - `checkUserWithCache.js` — обновление кэша при необходимости, выбор валидной даты для пользователя;
...
- **Обработка ошибок (подход к Фазе 5):** Во всех `catch` и `.catch()` вместо прямого доступа к `error.message` используется `formatErrorForLog(err)` из `src/lib/utils.js` (или безопасный вариант `err?.message ?? String(err)`). Это исключает падения при не-Error значениях и унифицирует вывод в логах. VisaProviderFactory при неизвестном `providerId` выбрасывает явную ошибку вместо тихого fallback на AIS. На границе CLI (`src/index.js`): обработчик `unhandledRejection` и `parseAsync().catch()` логируют через `formatErrorForLog` и завершают процесс с кодом 1 без сырого stack.
- **Паритет путей инициализации:** В `monitor.js` при успешной загрузке composition root явно вызываются `initializeTelegram(config.telegramBotToken, config.telegramManagerChatId)` и `setSheetsQuotaNotifier(...)`, так что глобальный Telegram и уведомления о квоте Sheets работают так же, как в fallback-пути (и уведомления из UserBotManager, и квотные алерты доходят до Telegram).
- **SheetsUserRepository:** опциональные параметры `rowIndex`, `timeSlot` передаются в sheets без нормализации (`null`/`undefined` не заменяются через `??`), чтобы не менять семантику при разной трактовке в будущем. В `sheets.js` в JSDoc для соответствующих функций указаны типы `number|null|undefined` и `string|null|undefined`.
