...
> **Mirror:** identical skill in `.agents/skills/deploy-us-visa-bot/SKILL.md` (Cursor loads both `.cursor/skills/` and `.agents/skills/`).
...
**Runtime note:** if Sheets have no **active=TRUE** users, `monitor` idles and polls Sheets (no crash loop under systemd). See `DEPLOY.md` § “Monitor: empty sheet or no active users”.
